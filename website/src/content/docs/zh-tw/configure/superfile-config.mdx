...
:::note
你需要確認 powershell 允許執行 script。如果出現像 `running
scripts is disabled on this system` 這樣的錯誤，就需要允許它。
...
:::caution
即使已啟用額外欄位，如果 filepanel 尺寸太小，欄位仍會被隱藏。
如果看不到欄位，請增加大小或調整 file_panel_name_percent 設定。
:::
...
`''` => 使用內建的 _chroma_ 語法醒目提示來預覽程式碼檔案。
`'bat'` => 使用 [`bat`](https://github.com/sharkdp/bat) 命令列工具提供的語法醒目提示。
